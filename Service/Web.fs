...
open DrizzleCarton.Service.Serialization
...
module Web =
  ...
  type CreateUser = { Name: string; Quota: int }
  ...
  let webApp: HttpHandler =
    choose
      [ route "/user" >=> users
        ...
        GET >=> routef "/user/%i" user

        PUT >=> routef "/user/%i/username" renameUser

        GET >=> routef "/user/%i/file" entries ]
